...
Flera steg krävs när Adobe Analytics implementeras som rapportkälla för Target (A4T).

## Implementeringssteg {#section_73961BAD5BB4430A95E073DE5C026277}

I följande tabell beskrivs stegen som krävs för att distribuera den här integreringen till din plats.

## Steg 1: Begär etablering för Analytics och Target

När ni har implementerat Analytics som rapportkälla för Target måste ni ha tillgång till Analytics och Target. [Använd det här formuläret om du vill begära att få etableras](http://www.adobe.com/go/audiences).

## Steg 2: Konfigurera användarbehörigheter

Kraven för användarkontot måste uppfyllas innan du kan skapa en Adobe Analytics-baserad aktivitet i Adobe Target. Se Krav för [användarbehörighet](/help/c-integrating-target-with-mac/a4t/account-reqs.md).

## Steg 3: Implementera tjänsten Experience Cloud Visitor ID

Med besökar-ID-tjänsten kan du identifiera användare i olika Experience Cloud-lösningar. Du måste implementera eller migrera till den version av Experience Cloud Visitor ID som krävs. Mer information finns i Implementeringskrav i [Innan du implementerar](/help/c-integrating-target-with-mac/a4t/before-implement.md).
...
Det sätt på vilket de färdiga inställningarna konfigureras för Target- och Analytics-integrering från ett implementeringsperspektiv är att använda det SDID som skickas från sidan för att sammanfoga Target- och Analytics-begäran automatiskt.

Men om ni vill ha större kontroll över hur och när analysdata som rör Target ska skickas till Analytics för rapportering, och ni inte vill använda standardinställningarna för att låta Target och Analytics automatiskt sammanfoga analysdata via SDID, kan ni ange **analyticsLogging = client_side** via **window.targetGlobalSettings**. Obs! versioner under 2.1 stöder inte detta tillvägagångssätt.
...
Den här konfigurationen har en global effekt, vilket innebär att alla anrop från at.js har **analyticsLogging: &quot;client_side&quot;** skickas i Target-begäranden och en analysnyttolast returneras för varje begäran. När detta är konfigurerat ser nyttolastens format ut så här:
...
Läs in sidorna när du har uppdaterat JavaScript-biblioteken för att bekräfta att parametervärdena mboxMCSDID i Target-anrop matchar parametervärdet i sidvisningsanropet för Analytics.
...
I Mål klickar du på [!UICONTROL Setup > Preferences] och väljer antingen [!UICONTROL Select per activity] eller [!UICONTROL Adobe Analytics] för att aktivera alternativen.

* Välj per aktivitet för att välja mellan Mål och Analytics när du skapar varje aktivitet.
* Adobe Analytics anger Analytics som rapportkälla för alla aktiviteter ni skapar.
