...
1. Konfigurera följande inställningar:
   ...
   * **Visa standardinnehåll för alla andra aktiviteter:** Om det här alternativet är aktiverat visas standardinnehåll för alla andra aktiviteter (förhandsvisningen visas till exempel separat utan att alla andra aktiva aktiviteter på samma sida/[!DNL Target] begäran beaktas).

      Om den här inställningen är inaktiverad bör du tänka på följande:

      * Om det finns kollisioner mellan aktiviteten som du testar och andra aktiva aktiviteter gäller [normala prioritetsregler](/help/c-activities/priority.md#concept_1780C11FEA57440499F0047DD6900E0F). På grund av detta kan det hända att du inte kan se aktiviteten som du tänker göra till en kvalitetskontroll.
...
* Om du har .js 1.*x*, Activity QA-läget är inte tilltalande om du använder Safari eller en annan webbläsare som blockerar cookies från tredje part. I dessa fall måste du lägga till förhandsgranskningsparametrarna i varje URL som du navigerar till. Detsamma gäller om du har implementerat [CNAME](/help/c-implementing-target/c-considerations-before-you-implement-target/implement-cname-support-in-target.md).
...
* Alla `at_preview`-parametrar och värden är redan URL-kodade. För det mesta fungerar allt som förväntat. Vissa kunder kan dock ha belastningsutjämnare eller webbservrar som försöker koda frågesträngsparametrarna igen.
   ...
   På grund av den här dubbla kodningen kan Target inte extrahera rätt tokenvärde när vi försöker avkoda `at_preview_token`, vilket resulterar i att förhandsvisningen inte fungerar.
   ...
   | `at_preview_evaluate_as_true_audience_ids` | Sträng | Tom | Understreckavgränsad (&quot;_&quot;) lista med segmentId-s som alltid (på mål- och rapporteringsnivå) ska utvärderas som &quot;true&quot; i omfattningen av [!DNL Target]-begäran. |
   ...
   | `adobe_mc_sdid` |  |  | Skickar [!DNL Supplemental Data Id] (SDID) och [!DNL Experience Cloud Org Id] från standardsidan till den nya sidan så att Analytics for Target (A4T) kan sammanfoga Target-begäran på standardsidan med Analytics-begäran på den nya sidan. |
