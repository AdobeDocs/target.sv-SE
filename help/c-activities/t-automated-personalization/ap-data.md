...
| [!DNL Target] profil | PRO | Anpassade profilattribut som har överförts direkt till [!DNL Target] profil via API eller sidparameter och [!DNL Target] profilskript. | Custom - Visitor Profile - [attribute name] |
...
| Attributnamn | Attributbeskrivning | Exempelvärden | Systemnamn |
| --- | --- | --- | --- |
| Geo - ort | Den stad som besökaren kom åt aktiviteten från. | San Francisco | Geo_City |
...
| Besöksprofil - livstid för aktivitet på plats | Specifies the visitor&#39;s total time spent on site, excluding the current session, and is updated when the session expires. | Dubbla, millisekunder | SES_TOTAL_TIME |
| Visitor Profile -Average Page Views per Visit during Activity | Anger det genomsnittliga antalet sidvisningar per session, exklusive den aktuella sessionen. | Dubbel | SES_REQUESTS_PER_SESSION |
| Besöksprofil - genomsnittlig tid per besök | Anger den genomsnittliga tiden per besök/session. Detta inkluderar inte den aktuella sessionen. | Double, milliseconds | SES_TIME_PER_SESSION |
| Besöksprofil - första besök | Anger tidpunkten för det första besök som användaren interagerade med [!DNL Target]. | Double, milliseconds | SES_PROFILE_CREATION_TIME |
| Besöksprofil - timmar sedan senaste besök | Specifies the hours since the last visit to this particular activity. | Dubbel (endast positivt heltal) 1, 2, 3 osv. | SES_HOURS_SINCE_LAST_VISIT |
...
| Besöksprofil - Totalt antal besök till aktivitet | Anger antalet besök för en viss aktivitet. If there is no previous visit, returns 0 (zero). | Double (Only integer positive number) 1, 2, 3, etc. | SES_PREVIOUS_VISIT_COUNT |
...
| Visitor Profile - Visits to Activity with No Conversion | Antal besök/sessioner utan konverteringar till en viss aktivitet. Det här värdet återställs till noll efter konvertering eller till -1 om konvertering aldrig inträffar. | Dubbel (endast positivt heltal) 1, 2, 3 osv. | SES_SUCCESS_RECENCY |
