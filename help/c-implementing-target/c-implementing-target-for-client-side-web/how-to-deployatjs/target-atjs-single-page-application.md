...
description: Lär dig använda Adobe [!DNL Target] at.js 2.x to implement [!DNL Target] för enkelsidiga program (SPA).
...
1. Installera på .js 2.x.
   ...
   Ladda ned på.js 2.x via Adobe Target-gränssnittet i [!UICONTROL Administration > Implementation]. at.js 2.x kan också distribueras via taggar i [!DNL Adobe Experience Platform]. Men [!DNL Adobe Target] tillägg är för närvarande inte aktuella och stöds inte.
...
Följande diagram hjälper dig att förstå arbetsflödet för at.js 2.x med Vyer och hur detta förbättrar SPA integrering. Om du vill få en bättre introduktion till de koncept som används i at.js 2.x kan du läsa [Implementering av Single Page-program](/help/c-implementing-target/c-implementing-target-for-client-side-web/how-to-deployatjs/target-atjs-single-page-application.md).
...
När du har installerat .js 2.x och lagt till `triggerView()` till webbplatsen använder du VEC för att köra A/B- och XT-aktiviteter. Mer information finns i [Visual Experience Composer (SPA) med en sida](/help/c-experiences/spa-visual-experience-composer.md).
...
Se till att [Analyser för Target](/help/c-integrating-target-with-mac/a4t/a4t.md) (A4T) fungerar korrekt med at.js 2.x, och du måste skicka samma SDID i Target-begäran och i Analytics-begäran.
...
at.js 2.x lade till ett nytt API [triggerView()](/help/c-implementing-target/c-implementing-target-for-client-side-web/adobe-target-triggerview-atjs-2.md) funktion. Du bör använda `triggerView()` att meddela at.js att en vy kommer att starta återgivningen.
...
### Så här fungerar at.js 2.x ![Märket Översikt](/help/assets/overview.png)
...
### Använda VEC för SPA i Adobe Target ![Självstudiemärke](/help/assets/tutorial.png)
