...
| ikonen [!UICONTROL Show filters]<P>![Ikonen Visa filter](/help/main/assets/icons/Filter.svg) | Du kan få åtkomst till filter genom att klicka på ikonen **[!UICONTROL Show Filters]** i den övre delen av listan för att filtrera aktiviteter efter [!UICONTROL Type], [!UICONTROL Status], [!UICONTROL Reporting Source], [!UICONTROL Experience Composer], [!UICONTROL Metrics Type], [!UICONTROL Activity Source] och [!UICONTROL Properties].<P>Mer information finns i [Använda filter i aktivitetslistan](#filters) nedan. |
| Sökfält | Hitta snabbt en aktivitet eller minska antalet aktiviteter som visas i listan [!UICONTROL Activity]. Du kan söka efter [!UICONTROL Activity Name], [!UICONTROL URL] eller [!UICONTROL ID]. |
