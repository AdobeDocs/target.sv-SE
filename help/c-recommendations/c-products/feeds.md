...
Använd feeds för att importera enheter till [!DNL Recommendations]. Enheter kan skickas med CSV-filer, Googles produktsökningsformat och Adobe Analytics produktklassificeringar.
...
Du kan välja vilka kolumner från din [!DNL Target] produktklassificeringsfil eller Google Product Search-fil som du vill skicka till [!DNL Recommendations] servern. Dessa datadelar om varje objekt kan sedan användas för mallvisning och för att styra rekommendationer.

Om data samlas in både av en entitetsfeed och en mbox, vinner den senaste informationen. Vanligtvis kommer de senaste data från en mbox, eftersom de visas oftare. I den sällsynta händelse att enhetsmatningsdata och mbox-data träffar samtidigt, används mbox-data.
